...
void loop()
{
  ...
  blockNum = 5;
  toBlockDataArray("Onur_Akyildiz"); //First Name
  WriteDataToBlock(blockNum, block_data);
  ReadDataFromBlock(blockNum, readBlockData);
  dumpSerial(blockNum, readBlockData);

  blockNum = 6;
  toBlockDataArray("Kocaeli_Uni"); //Last Name
  WriteDataToBlock(blockNum, block_data);
  ReadDataFromBlock(blockNum, readBlockData);
  dumpSerial(blockNum, readBlockData);

  blockNum = 8;
  toBlockDataArray("Teknoloji_fak"); //Faculty Name
  WriteDataToBlock(blockNum, block_data);
  ReadDataFromBlock(blockNum, readBlockData);
  dumpSerial(blockNum, readBlockData);

  blockNum = 9;
  toBlockDataArray("Bil_Sis_Muh"); //Department Name
  WriteDataToBlock(blockNum, block_data);
  ReadDataFromBlock(blockNum, readBlockData);
  dumpSerial(blockNum, readBlockData);
